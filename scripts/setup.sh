...
# WARNING: This file contains secrets. Keep permissions restricted (chmod 600).
...
PORTFOLIOEOF
    ...
    chmod 600 "$env_file"
    log_success ".env written to $env_file (permissions: 600)"
}
...
phase_verification() {
    ...
    if [[ -f "$PROJECT_ROOT/.env" ]]; then
        ...
        local perms
        perms=$(stat -f "%Lp" "$PROJECT_ROOT/.env" 2>/dev/null || stat -c "%a" "$PROJECT_ROOT/.env" 2>/dev/null || echo "unknown")
        if [[ "$perms" == "600" ]]; then
            check_result ".env permissions (600)" "pass"
        else
            check_result ".env permissions (got $perms, want 600)" "warn"
        fi
    else
