...
class PlatformAPI:
    ...
    def adjust_prices_for_dividends(
        ...
        For each dividend, adjusts all prior prices by factor: 1 - (dividend / price_on_ex_date).
        Works backward from most recent to oldest dividend.

        Args:
            prices: DataFrame with columns [symbol, date, close, ...]
            reinvest: If True, assumes dividend reinvestment (default).
                      If False, returns prices unchanged (price return only).

        Returns:
            DataFrame with adjusted prices (total return basis) plus
            'dividend_adjusted_close' column.

        Note:
            This is typically used for total return analysis. For price-only
            backtests, use unadjusted prices (reinvest=False).
        """
        if prices.empty:
            logger.warning("Empty prices DataFrame provided to adjust_prices_for_dividends")
            return prices.assign(dividend_adjusted_close=None)
        ...
        # Initialize dividend_adjusted_close with close price
        df["dividend_adjusted_close"] = df["close"].copy()

        # If reinvest=False, return prices unchanged
        if not reinvest:
            logger.debug("reinvest=False, returning prices unchanged")
            return df
        ...
        # Get all dividend actions for these symbols
        dividends = self.get_corporate_actions(symbols, start_date, end_date)

        # Filter to only dividends
        if not dividends.empty:
            dividends = dividends[dividends["action_type"] == "dividend"].copy()
            dividends["date"] = pd.to_datetime(dividends["date"])

        if dividends.empty:
            logger.debug("No dividends found, returning prices with unadjusted close")
            return df

        # Apply dividends symbol by symbol
        for symbol in symbols:
            symbol_dividends = dividends[dividends["symbol"] == symbol].sort_values(
            ...
            # Apply each dividend backward in time
            for _, dividend in symbol_dividends.iterrows():
                ex_date = dividend["date"]
                dividend_amount = dividend["factor"]

                # Find the price on the ex-dividend date (or closest prior date)
                symbol_prices = df[symbol_mask].copy()
                prices_on_ex_date = symbol_prices[symbol_prices["date"] == ex_date]

                if prices_on_ex_date.empty:
                    # Try to find closest prior date
                    prior_prices = symbol_prices[symbol_prices["date"] < ex_date]
                    if not prior_prices.empty:
                        price_on_ex_date = prior_prices.iloc[-1]["close"]
                    else:
                        # No price available, skip this dividend
                        logger.warning(
                            f"No price found for {symbol} on or before ex-date {ex_date}"
                        )
                        continue
                else:
                    price_on_ex_date = prices_on_ex_date.iloc[0]["close"]

                # Calculate adjustment factor: 1 - (dividend / price)
                adjustment_factor = 1 - (dividend_amount / price_on_ex_date)

                # Adjust all prices BEFORE the ex-dividend date
                date_mask = df["date"] < ex_date
                mask = symbol_mask & date_mask

                df.loc[mask, "dividend_adjusted_close"] *= adjustment_factor

            logger.debug(f"Applied {len(symbol_dividends)} dividends for {symbol}")

        logger.debug(f"Dividend adjustment complete for {len(symbols)} symbols")
        return df
