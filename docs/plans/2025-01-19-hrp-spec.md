...
- Repository setup (git, pyproject.toml, pre-commit)
- CLAUDE.md and README.md
- Virtual environment with core dependencies
- DuckDB schema creation (`hrp/data/schema.py`)
- Basic data source adapter (Yahoo Finance)
- Price ingestion for 10 test symbols
...
- 10 symbols with 5+ years of price data loaded
- Schema matches spec
- All tests pass
...
- Platform API skeleton (`hrp/api/platform.py`)
...
- Results logged to MLflow with params, metrics, artifacts
- Equity curve artifact generated
...
- Lineage system (`hrp/research/lineage.py`)
- Link hypotheses to experiments
- Hypothesis CRUD via Platform API
- Basic validation criteria checks
...
- Hypothesis lifecycle works (draft → testing → validated/rejected)
- All experiments linked to hypotheses
- Lineage queries return correct chains
...
- Streamlit app skeleton (`hrp/dashboard/app.py`)
- Home page (system status)
- Data Health page (ingestion status)
- Hypotheses page (list, create, view)
- Experiments page (MLflow integration)
...
- Can view hypotheses and experiments
- Can trigger backtest from UI
...
- S&P 500 universe management (`hrp/data/universe.py`)
- Polygon.io integration (`hrp/data/sources/polygon_source.py`)
- Feature store implementation (`hrp/data/ingestion/features.py`)
- Data quality checks (`hrp/data/quality/checks.py`)
- Scheduled ingestion with APScheduler
- Universe filtering (exclude financials, REITs, penny stocks)

#### Optional Enhancements

- YFinance fallback source
- Corporate actions support (splits, dividends)
- Quality report storage and health trends
...
- Incremental feature computation — Performance optimization for large-scale updates
...
- ML model registry (`hrp/ml/models.py`) — Ridge, Lasso, ElasticNet, RandomForest, MLP + optional LightGBM/XGBoost
- Training pipeline (`hrp/ml/training.py`) — Data loading, train/val/test splits, metrics
- Walk-forward validation (`hrp/ml/validation.py`) — Expanding/rolling windows, stability score
- Feature selection — Mutual information based
- Signal generation from predictions (`hrp/ml/signals.py`) — rank, threshold, zscore methods
- Overfitting guards (test set discipline)
...
- Linear models (Ridge, Lasso) working
- Tree-based models (RandomForest) working
- Walk-forward validation implemented
- Test set guard enforced
- ML experiments logged to MLflow
...
- Job infrastructure (`hrp/agents/jobs.py`) — PriceIngestionJob, UniverseUpdateJob, FeatureComputationJob
- CLI interface (`hrp/agents/cli.py`)
- Email notifications (`hrp/notifications/email.py`)
- Data Monitor agent
...
- Test set discipline (TestSetGuard with 3-evaluation limit)
- Strategy validation gates (`hrp/risk/validation.py`)
...
- Cannot exceed 3 test set evaluations per hypothesis
...
| 9 | Paper Trading | Live deployment |
