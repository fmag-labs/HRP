...
- [ ] Polygon.io integration (or chosen paid source)
...
python -m hrp.data.ingestion.prices --date today
python -m hrp.data.quality.check --report
```
...
- [ ] ML model registry (`hrp/ml/models.py`)
- [ ] Training pipeline (`hrp/ml/training.py`)
...
config = MLConfig(model_type='lightgbm', target='return_20d', ...)
experiment_id = train_model(config)
signals = generate_signals(experiment_id, date.today())
```

#### Exit Criteria

- LightGBM and linear models working
